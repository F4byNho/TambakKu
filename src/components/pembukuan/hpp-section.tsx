...
export default function HPPSection() {
  ...
  const targetAnggotaId = activeAnggota?.anggota_id || activeTambak?.anggota_id || "";

  // Fetch Tambak (Filtered by currently active personal context)
  useEffect(() => {
    const fetchTambak = async () => {
      setIsLoadingTambak(true);
      try {
        const url = targetAnggotaId 
          ? `/api/tambak?anggotaId=${targetAnggotaId}`
          : "/api/tambak";
        const res = await fetch(url);
        const json = await res.json();
        if (!res.ok) throw new Error(json.error);
        const list: Tambak[] = json.data || [];
        setTambaks(list);
      } catch (e: any) {
        setError(e.message);
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border border-slate-200/90 shadow-2xs rounded-2xl bg-white p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="space-y-1.5">
            <label htmlFor="tambak" className="text-xs font-bold text-slate-700">Kolam / Tambak</label>
            <select
              id="tambak"
              value={selectedTambakId}
              onChange={(e) => setSelectedTambakId(e.target.value)}
              disabled={isLoadingTambak}
              className="w-full h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-600 disabled:bg-slate-50 disabled:text-slate-400 shadow-2xs"
            >
              <option value="" disabled>
                {isLoadingTambak ? "Memuat..." : "Pilih tambak..."}
              </option>
              {(() => {
                const displayTambaks = targetAnggotaId
                  ? tambaks.filter((t) => t.anggota_id === targetAnggotaId)
                  : tambaks;

                return displayTambaks.map((t) => (
          ...
          {/* Siklus */}
          <div className="space-y-1.5">
            <label htmlFor="siklus" className="text-xs font-bold text-slate-700">Siklus Budidaya</label>
            <select
              id="siklus"
      ...
      {!selectedSiklusId && !isLoadingHPP && !error && (
        <div className="flex flex-col items-center gap-3 py-16 text-slate-400 bg-white border border-slate-100 rounded-2xl">
          <Calculator className="h-12 w-12 text-slate-200" />
          <p className="text-sm font-medium text-center">
            Pilih tambak dan siklus budidaya untuk melihat laporan HPP &amp; simulasi harga jual
          </p>
        </div>
      )}
          ...
          {tabs.length > 0 && (
            <div className="bg-slate-100/80 p-1.5 rounded-2xl border border-slate-200 shadow-2xs">
              <div className="flex items-center justify-between gap-1.5 overflow-x-auto">
                ...
                <button
                  onClick={fetchHPP}
                  className="shrink-0 rounded-xl p-2.5 border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-2xs"
                  title="Refresh data"
                >
